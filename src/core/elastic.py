...
class ElasticClient:
    ...
    async def index_custom_chunks(
        ...
        logger.info("Indexed %d chunks for document %s", success_count, document_id)
    ...
    async def search_custom_documents(
        ...
        """kNN search over custom document chunks, filtered by tenant and document IDs."""
        idx = settings.custom_documents_index
    ...
    async def delete_custom_document(self, document_id: str, tenant_id: str) -> int:
    ...
    async def delete_custom_documents_by_tenant(self, tenant_id: str) -> int:
        ...
        deleted = resp.get("deleted", 0)
        logger.info("Deleted %d custom chunks for tenant %s", deleted, tenant_id)
        return deleted
